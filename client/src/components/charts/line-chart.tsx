...
export function LineChart({
  ...
  // Support both single data array and multiple series
  const isMultiSeries = !!series && series.length > 0;
  const dataPoints = isMultiSeries ? series[0]?.data || [] : data || [];
  ...
  const maxValue = isMultiSeries
    ? Math.max(...(series?.flatMap((s) => s.data.map((d) => d.value)) || []), 1)
  ...
  // If title is empty, don't render Card wrapper
  const content = (
    <div className={cn("w-full", className)}>
      <div className="overflow-x-auto w-full">
        ...
        >
          ...
          {/* Render multiple series or single line */}
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="text-xs">Value: {point.value}</div>
                        </div>
                      </TooltipContent>
                    </Tooltip>
                  </g>
  ...
  // If title is empty, return content without Card wrapper
  if (!title) {
    return content;
  }
