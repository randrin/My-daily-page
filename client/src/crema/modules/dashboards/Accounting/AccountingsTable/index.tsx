...
import { useAccountingActionsContext, useAccountingContext } from "modules/apps/context/AccountingContextProvider";
import {
...
const getColumns = (router: NextRouter): ColumnsType<ReportExpenseType> => [
  ...
  },
  {
    ...
    render: (id, record) => (
      <StyledFlex>
        {record.expense.approver ? (
          <>
            {record.expense.approver?.photoURL ? (
              <Avatar
                style={{
                  marginRight: 14,
                  width: 44,
                ...
                src={record.expense.approver?.photoURL.secure_url}
              />
            ) : (
              <StyledUserInfoAvatar
                photoRGA={record.expense.approver?.photoRGA}
              >
                {Tt_GetUserAvatar(record.expense.approver)}
              </StyledUserInfoAvatar>
            )}
            <div
              style={{
            ...
            >
              ...
              >
                {record.expense.approver?.displayName}
              </Typography.Title>
              <StyledText>{record.expense.approver?.email}</StyledText>
            </div>
          </>
        ) : (
          "-"
        ...
        {record.expense.employee ? (
          <>
            {record.expense.employee?.photoURL ? (
              ...
              <StyledUserInfoAvatar photoRGA={record.expense.employee.photoRGA}>
...
const AccountingTable = ({ reportData, loading }: Props) => {
  ...
  const { isReject, rejectComment } = useAccountingContext();
  const { setIsReject, setRejectComment, handleOnAccountingExpense } =
